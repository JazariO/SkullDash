...
public class PlayerCameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            float tilt_target = Mathf.InverseLerp(0f, player_settings_SO.runSpeed, player_stats_SO.tempStats.speed);
            if(Mathf.Abs(tilt_target) < 0.01f) tilt_target = 0f;

            float tilt_target_direction = -Vector3.Dot(player_stats_SO.tempStats.moveVelocity, pivot_camera_end_effector.right);
            tilt_target *= tilt_target_direction;
